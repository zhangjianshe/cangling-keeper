...
- 软件仓库：可管理多个软件集（缺省包含维护中心 Manifest 集 `np4`、`images`，以及 Git 仓库 `cangling-repo`、`cangling-np4`）。软件集分为两种：向维护中心查询 manifest 后按哈希下载，或自定义 Git 仓库 clone/pull。同步大文件时显示下载/克隆进度，并可浏览目录与文本文件内容。
