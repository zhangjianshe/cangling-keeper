        ...
        let changed = self
            ...
            .execute(
                "UPDATE hosts SET name=?1, hostname=?2, port=?3, username=?4,
                        ...
                        update_port=?13, update_role=?14 WHERE id=?15",
                params![
                    host.name,
                    host.hostname,
                    host.port,
...
fn migrate(conn: &Connection) -> rusqlite::Result<()> {
    ...
    ensure_column(
        ...
        "tunnels",
        "local_host",
        "ALTER TABLE tunnels ADD COLUMN local_host TEXT NOT NULL DEFAULT '127.0.0.1'",
    )?;
    ensure_column(
        conn,
        "hosts",
    ...
    ensure_column(
        ...
        "hosts",
        "update_role",
        "ALTER TABLE hosts ADD COLUMN update_role TEXT NOT NULL DEFAULT 'standalone'",
    )?;
    ensure_column(
        conn,
        "hosts",
...
fn host_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<Host> {
    ...
    let update_role: String = row.get(14)?;
    Ok(Host {
        id: row.get(0)?,
        name: row.get(1)?,
        hostname: row.get(2)?,
